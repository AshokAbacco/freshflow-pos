...
import { LuCircleCheck, LuCloudUpload, LuPrinter, LuReceipt, LuSend, LuUsb } from 'react-icons/lu';
...
function ThermalReceipt({ r }) {
  ...
  return (
    <div className="mx-auto w-[72mm] font-mono text-[11px] leading-[1.35] text-black">
      ...
      <p className="my-1">{'-'.repeat(42)}</p>
      <p className="text-center">{r.store?.receiptFooter || 'Thank you for shopping with us.'}</p>
      {r.pending ? <p className="mt-1 text-center">Bill saved on this register; syncs automatically</p> : null}
    </div>
  );
}

export function ReceiptModal({ receipt, onClose }) {
  const { printer, printBytes, connectPrinter, serialSupported } = useHardwareStore();
    ...
    <>
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              ...
              {receipt.customerPhone ? (
                <Button
                  variant="secondary"
                  onClick={() => window.open(whatsappLink(receipt.customerPhone, receiptToWhatsAppText(receipt)), '_blank', 'noopener')}
                >
                  <LuSend aria-hidden /> WhatsApp
                </Button>
              ) : (
                <Button variant="secondary" onClick={printThermal} loading={sending} disabled={!serialSupported}>
      ...
      >
        <div className="flex flex-col items-center text-center">
          <div className="grid h-12 w-12 place-items-center rounded-full bg-brand-50 text-brand">
            <LuCircleCheck size={24} aria-hidden />
          </div>
          <p className="mt-3 text-h1 font-bold tabular">{formatMoney(receipt.totals.grandTotal, currency)}</p>
          ...
          {receipt.totals.discountTotal ? (
            <p className="mt-2 text-desc text-brand-700">Customer saved {formatMoney(receipt.totals.discountTotal, currency)}</p>
          ) : null}
          {receipt.pending ? (
            <p className="mt-3 flex items-center gap-1.5 rounded-xl bg-amber-50 px-3 py-2 text-desc text-amber-800">
              <LuCloudUpload aria-hidden /> Saved on this register. The invoice number is assigned when it uploads.
            </p>
          ) : null}
        </div>

        <details className="mt-5 rounded-2xl bg-slate-50 p-3">
        ...
        {printer.status === 'connected' && receipt.customerPhone ? (
          <button type="button" onClick={printThermal} className="mt-3 inline-flex items-center gap-1.5 text-desc font-semibold text-brand-700">
            <LuUsb aria-hidden /> Also print on the thermal printer
          </button>
        ) : null}
      </Modal>
      {createPortal(<ThermalReceipt r={receipt} />, document.getElementById('print-root'))}
    </>
  );
}
