...
export function ScaleModal({ product, currency, onClose, onAdd }) {
  ...
  const live =
    ...
    scale.reading &&
    Date.now() - scale.reading.at < LIVE_STALE_MS;
  ...
  return (
    <Modal
      ...
      description={
        live
          ? "Reading live from the connected scale"
          : "Enter or adjust the weight on the platter"
      }
      footer={
        <div className="flex items-center gap-3">
          <div className="min-w-0 flex-1">
            <p className="text-desc text-slate-500">Amount</p>
            <p className="text-h1 font-extrabold text-[#FF7B29] tabular">
    ...
    >
      ...
      >
          ...
          <div className="min-w-0">
            ...
            <p className="text-desc text-slate-500 tabular">
              ...
              {product.discountPercent > 0
                ? `, ${product.discountPercent}% markdown applied`
                : ""}
            </p>
          </div>
        </div>

        <div className="mt-4 rounded-xl bg-[#111] p-5 text-center shadow-inner">
          <p className="text-desc font-medium text-white/60">Net weight</p>
          <p
            className="mt-1 font-mono text-4xl font-bold text-brand tabular"
            aria-live="polite"
          >
            {net.toFixed(3)} kg
          </p>
          <div className="mt-2 flex justify-center gap-4 text-desc text-white/60 tabular">
            <span>Gross {gross.toFixed(3)} kg</span>
            <span>Tare {tare.toFixed(3)} kg</span>
          </div>
          <p className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-white/10 px-2.5 py-1 text-desc text-white/80">
            <span
              className={`h-1.5 w-1.5 rounded-full ${live ? (scale.reading.stable ? "bg-brand" : "animate-pulse bg-amber-500") : "bg-slate-400"}`}
              aria-hidden
            />
            {live
        ...
        <div className="mt-4 grid grid-cols-4 gap-2">
          <Button
            variant="secondary"
            onClick={() => setGross(manualGross - STEP_KG)}
            disabled={live || manualGross <= 0}
            className="!px-2"
          >
        ...
        {scale.error ? (
          <p className="mt-2 text-desc text-rose-600">{scale.error}</p>
        ) : null}
        {!serialSupported ? (
          <p className="mt-4 flex items-center gap-1.5 text-desc text-slate-500">
            <LuScale aria-hidden /> Live scale readings need Chrome or Edge on a
            computer.
          </p>
        ) : null}
      </div>
