 ...
 */
export function CategoryDensityChart({
  rows: allRows,
  currency,
  height = 260,
}) {
   ...
   */
  const rows = useMemo(
    () => allRows.filter((r) => r.revenue > 0 || r.prevRevenue > 0),
    [allRows],
  );
  ...
  const max = Math.max(
    1,
    ...rows.map((r) => Math.max(r.revenue, r.prevRevenue)),
  );
  ...
  const layout = useMemo(() => {
    ...
    const barW = Math.max(
      minBar,
      (width - (columns - 1) * gap) / columns - gap,
    );
  ...
  const Footnote = ({ children }) => (
    <p className="mt-2 text-desc text-slate-400">
      {children}
      {idleCount
        ? `${children ? " " : ""}${idleCount.toLocaleString("en-IN")} more had no sales in either period.`
        : ""}
    </p>
  );
  ...
  if (!rows.length) {
    return (
      <div ref={wrapRef}>
        <p className="py-6 text-center text-desc text-slate-500">
          No category sold anything in this period.
        </p>
      </div>
    );
  }
        ...
        <ul className="space-y-1.5">
          {rows.map((r) => {
            ...
            return (
              <li
                key={r.id}
                className="grid grid-cols-[minmax(96px,1.1fr)_3fr_auto] items-center gap-3"
              >
                <span
                  className="truncate text-desc text-slate-600"
                  title={r.parentName ? `${r.parentName} › ${r.name}` : r.name}
                >
                  {r.icon ? `${r.icon} ` : ""}
                  {r.name}
                </span>
                <span className="relative h-4 rounded-full bg-[#F1ECE4]">
                  <span
                  ...
                  {r.prevRevenue > 0 ? (
                    <span
                      ...
                      style={{ left: `calc(${prevPct}% - 1px)` }}
                      title={`Previous ${formatMoney(r.prevRevenue, currency)}`}
                    />
                  ) : null}
                </span>
                <span className="w-20 text-right text-desc font-bold text-gray-900 tabular">
  ...
  return (
    <div ref={wrapRef} className="relative w-full">
        ...
        <>
          ...
          >
            {rows.map((r, i) => {
              ...
              const barH =
                r.revenue > 0
                  ? Math.max(3, Math.sqrt(r.revenue / max) * track)
                  : 0;
              const prevH =
                r.prevRevenue > 0
                  ? Math.max(2, Math.sqrt(r.prevRevenue / max) * track)
                  : 0;
              ...
              return (
                <g key={r.id} onMouseEnter={() => setHover(i)}>
                  ...
                  <rect
                    x={x}
                    y={baseline - barH}
                    width={Math.max(1.5, layout.barW - 1)}
                    height={barH}
                    rx="1"
      ...
      {hover !== null && layout ? (
        <div
          className="pointer-events-none absolute z-10 max-w-[220px] rounded-lg bg-[#111] px-2.5 py-1.5 text-desc text-white shadow-lift"
          style={{
            left: Math.min(
              (hover % layout.columns) * layout.barW,
              Math.max(0, width - 220),
            ),
            top: Math.max(
              0,
              Math.floor(hover / layout.columns) * layout.bandH - 54,
            ),
          }}
        >
          <p className="truncate font-bold text-white">{rows[hover].name}</p>
          {rows[hover].parentName ? (
            <p className="truncate text-white/60">{rows[hover].parentName}</p>
          ) : null}
          <p className="font-bold text-brand tabular">
            {formatMoney(rows[hover].revenue, currency)}
          </p>
          <p className="text-white/60 tabular">
            was {formatMoney(rows[hover].prevRevenue, currency)}
          </p>
        </div>
      ) : null}
    </div>
