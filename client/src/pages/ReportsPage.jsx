        ...
        <div className="flex items-start gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-desc text-slate-500">
              ...
              {overview.data
                ? `, compared with the ${overview.data.period.days} day${overview.data.period.days === 1 ? "" : "s"} before`
                : ""}
            </p>
          </div>
      ...
      <div className="space-y-4 px-4 py-4 sm:px-6">
            ...
            : k
              ? [
                  <KpiCard
                    key="rev"
                    label="Revenue"
                    value={formatMoney(k.current.revenue, currency)}
                    current={k.current.revenue}
                    previous={k.previous.revenue}
                    hint={`${k.current.orders} bills`}
                  />,
                  <KpiCard
                    key="bask"
                    label="Average bill"
                    value={formatMoney(k.current.avgBasket, currency)}
                    current={k.current.avgBasket}
                    previous={k.previous.avgBasket}
                  />,
                  <KpiCard
                    key="items"
                    label="Items sold"
                    value={
                      k.current.units
                        ? formatNumber(k.current.units)
                        : `${formatNumber(k.current.weightKg)} kg`
                    }
                  ...
                  <KpiCard
                    ...
                    value={formatMoney(k.current.discounts, currency)}
                    current={k.current.discounts}
                    previous={k.previous.discounts}
                    invertColors
                    hint="passed to customers"
                  />,
                ]
              : null}
        </div>
        ...
        <div className="grid gap-4 xl:grid-cols-[1.6fr_1fr]">
          <section className="rounded-xl bg-white p-4 shadow-[0_10px_30px_-12px_rgba(0,0,0,0.15)] ring-1 ring-black/5 sm:p-5">
            <h2 className="font-extrabold text-gray-900">Revenue</h2>
            <p className="text-desc text-slate-500">
              Green is this period, grey is the one before
            </p>
            <div className="mt-3">
              {overview.data ? (
                <TrendChart
                  current={overview.data.series.current}
                  previous={overview.data.series.previous}
                  currency={currency}
                />
              ) : (
          ...
          <div className="space-y-4">
            <section className="rounded-xl bg-white p-4 shadow-[0_10px_30px_-12px_rgba(0,0,0,0.15)] ring-1 ring-black/5 sm:p-5">
              ...
              <div className="mt-3">
                {overview.data ? (
                  <SplitBar
                    data={overview.data.paymentSplit.current}
                    currency={currency}
                  />
                ) : (
            ...
            <section className="rounded-xl bg-white p-4 shadow-[0_10px_30px_-12px_rgba(0,0,0,0.15)] ring-1 ring-black/5 sm:p-5">
              <h2 className="font-extrabold text-gray-900">Stock on hand</h2>
              {inv ? (
                <>
                  <p className="mt-1 text-h1 font-extrabold text-gray-900 tabular">
                    {formatMoney(inv.value, currency)}
                  </p>
                  <p className="text-desc text-slate-500">
                    across {formatNumber(inv.skus)} products
                  </p>
                  <div className="mt-3 flex gap-2">
                    <span className="rounded-full bg-amber-50 px-2.5 py-1 text-desc font-medium text-amber-700 tabular">
                      {inv.lowStock} running low
                    </span>
                    <span className="rounded-full bg-rose-50 px-2.5 py-1 text-desc font-medium text-rose-700 tabular">
        ...
        <section className="rounded-xl bg-white p-4 shadow-[0_10px_30px_-12px_rgba(0,0,0,0.15)] ring-1 ring-black/5 sm:p-5">
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex-1">
              <h2 className="font-extrabold text-gray-900">Categories</h2>
              <p className="text-desc text-slate-500">
                {categories.data
                  ? `${formatNumber(categories.data.rows.length)} categories tracked`
                  : "Loading categories"}
              </p>
            </div>
            <Segmented
