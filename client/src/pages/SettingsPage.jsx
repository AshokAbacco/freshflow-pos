...
function Section({ title, description, children }) {
  return (
    <section className="rounded-xl bg-white p-4 shadow-[0_10px_30px_-12px_rgba(0,0,0,0.15)] ring-1 ring-black/5 sm:p-5">
      ...
      {description ? (
        <p className="mt-0.5 text-desc text-slate-500">{description}</p>
      ) : null}
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        {error ? (
          <p
            className="rounded-xl bg-rose-50 px-3 py-2 text-desc text-rose-700"
            role="alert"
          >
            {error}
          </p>
      ...
      <div className="mx-auto max-w-7xl space-y-4 px-4 py-4 sm:px-6">
        <Section title="Store details" description="Printed on every receipt">
          <div className="space-y-4">
            ...
            <div>
              <label htmlFor="s-address" className="label">
                Address
              </label>
              <input
                id="s-address"
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                <label htmlFor="s-gstin" className="label">
                  GSTIN
                </label>
                <input
                  id="s-gstin"
            ...
            <div>
              <label htmlFor="s-footer" className="label">
                Receipt footer
              </label>
              <input
                id="s-footer"
        ...
        <Section
          ...
          description="Used to calculate bills and build the UPI QR code"
        >
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
            ...
            <div>
              <label htmlFor="s-payee" className="label">
            ...
            <div>
              <label htmlFor="s-tax" className="label">
                GST rate (%)
              </label>
              <input
                id="s-tax"
            ...
            <div>
              <label htmlFor="s-cap" className="label">
              ...
              <input
                ...
                inputMode="decimal"
                className="field tabular"
                value={form.maxCashierDiscountPct ?? ""}
                onChange={set("maxCashierDiscountPct")}
              />
            </div>
          </div>
          {error ? (
        ...
        <Section
          title="Hardware on this register"
          description="Connections are per device and stay until you close the browser"
        >
          <div className="space-y-2">
            {[
              {
                ...
                hint: "Reads live weight over USB or RS-232",
              },
              {
                ...
                hint: "Prints 80 mm ESC/POS receipts",
              },
            ].map(({ label, icon: Icon, state, connect, disconnect, hint }) => (
              <div
                key={label}
                className="flex items-center gap-3 rounded-lg bg-[#F8F4EE] px-3 py-2.5"
              >
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-white text-brand ring-1 ring-black/5">
                  <Icon aria-hidden />
                </span>
                <div className="min-w-0 flex-1">
            ...
            {!serialSupported ? (
              <p className="text-desc text-slate-500">
                Hardware connections need Chrome or Edge on a computer. You can
                still print receipts from the browser print dialog.
              </p>
            ) : null}
          </div>
        </Section>
        ...
        <Section
          title="Team"
          description="Cashiers can use the register. Admins also see reports, inventory and settings."
        >
          ...
          <ul className="divide-y divide-slate-100">
            {(users.data?.users ?? []).map((u) => (
              <li key={u.id} className="flex items-center gap-3 py-2.5">
                <span
                  className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-brand-50 text-desc font-bold text-brand-700"
                  aria-hidden
                >
                ...
                <button
                  ...
                  className={`rounded-full px-2.5 py-1 text-desc font-medium ${u.role === "ADMIN" ? "bg-brand-50 text-brand-700" : "bg-slate-100 text-slate-600"}`}
                >
